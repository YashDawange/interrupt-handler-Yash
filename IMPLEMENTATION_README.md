...
- **Key Methods**:
  - `_is_ignored_transcript()` (lines 167-189): Validates if transcript contains only ignored words
  - `_interrupt_by_audio_activity()` (lines 1193-1255): Handles VAD-based interruptions with state-aware filtering
  - `on_end_of_turn()` (lines 1389-1445): Processes end-of-turn events with intelligent filtering
...
- ✅ `test_is_ignored_transcript`
- ✅ `test_interrupt_by_audio_activity_speaking_vad_only`
- ✅ `test_interrupt_by_audio_activity_speaking_ignored_word`
- ✅ `test_interrupt_by_audio_activity_speaking_valid_word`
- ✅ `test_on_end_of_turn_speaking_ignored_word`
- ✅ `test_on_end_of_turn_silent_ignored_word`
...
uv sync
# or
pip install -e ./livekit-agents[openai,deepgram,silero]
```
...
**Option 3: Console Mode (Text)**
```bash
python examples/voice_agents/basic_agent.py console --text
```

**Option 4: Dev Mode (Voice with LiveKit)**
```bash
...
- ✅ Agent continues speaking seamlessly when user says backchanneling words
- ✅ Agent correctly interrupts on real commands
- ✅ Agent processes backchanneling words as valid input when silent

...
class AgentActivity(RecognitionHooks):
    ...
    def _interrupt_by_audio_activity(self) -> None:
        ...
        # BACKCHANNEL FILTER - Check if current speech exists and should be filtered
        if (
            self.stt is not None
            and self._audio_recognition is not None
            ...
            # Check for backchannel vs interrupt words using substring matching
            has_wait = 'wait' in text
            has_stop = 'stop' in text
            has_no = ' no' in text or 'no ' in text
            has_interrupt = has_wait or has_stop or has_no or 'hold' in text or 'pause' in text
            
            has_yeah = 'yeah' in text
            has_ok = 'ok' in text or 'okay' in text
            has_hmm = 'hmm' in text or 'hm' in text or 'um' in text or 'uh' in text
            has_backchannel = has_yeah or has_ok or has_hmm or 'right' in text or 'aha' in text
            
            # If ONLY backchannel detected (no interrupt words), ignore
            if has_backchannel and not has_interrupt and len(text) < 15:
                ...
                if text:
                    ...
                    if has_backchannel and not has_interrupt:
                        ...
                        if len(remaining.strip()) < 3:
                            logger.info(f"✋ IGNORING backchannel '{text}' during speech")

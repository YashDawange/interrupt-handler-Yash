...
logger = logging.getLogger("acknowledgment-patch")
...
def _is_acknowledgment_only(text: str) -> bool:
    """Check if text contains ONLY acknowledgment words."""
    ...
    for word in words:
        if word in DIRECTIVE_WORDS:
            return False
            
    # check all words
    for word in words:
        if word in ACKNOWLEDGMENT_WORDS:
            continue
        if '-' in word:
            parts = word.split('-')
            if all(p in ACKNOWLEDGMENT_WORDS for p in parts if p):
                continue
        return False
        
    return True

def _check_acknowledgment_delta(full_transcript: str) -> bool:
    delta = _get_transcript_delta(full_transcript)
    if not delta:
        return True # Empty delta is safe
        
    is_ack = _is_acknowledgment_only(delta)
    if is_ack:
        logger.info(f"🛡️ [PATCH FILTER] Delta '{delta}' is acknowledgment - BLOCKING")
    else:
        logger.debug(f"✅ [PATCH FILTER] Delta '{delta}' is NOT acknowledgment - allow")
    return is_ack
...
def patched_interrupt_by_audio_activity(self) -> None:
    ...
    if (
        ...
        # --- PATCH START ---
        if _check_acknowledgment_delta(text):
            logger.debug(f"🚫 [PATCH FILTER] Blocking AUDIO interruption - acknowledgment delta")
            return
        # --- PATCH END ---
...
def patched_on_end_of_turn(self, info: _EndOfTurnInfo) -> bool:
    ...
    if (
        self._current_speech is not None 
        and not self._current_speech.interrupted
        and _check_acknowledgment_delta(info.new_transcript)
    ):
        self._cancel_preemptive_generation()
        logger.info(f"🚫 [PATCH FILTER] EndOfTurn blocked - acknowledgment delta")
        return False
...
def apply_override():
    logger.info("Applying Acknowledgment Override to AgentActivity...")
